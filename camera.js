class Camera {
    ...
    #extrudeCar(poly, height = 20, wheelRadius = 5) {
        ...
        const middleLeft = new Point(
            (frontLeft.x + backLeft.x) / 2,
            (frontLeft.y + backLeft.y) / 2
        );
        const middleRight = new Point(
            (frontRight.x + backRight.x) / 2,
            (frontRight.y + backRight.y) / 2
        );
        ...
        const c_middleLeft = ceiling.points[1];
        const c_quarterFrontLeft = ceiling.points[2];
        //middleLeft,
        const c_quarterBackLeft = ceiling.points[3];
        const c_backLeft = ceiling.points[4];
        const c_backRight = ceiling.points[5];
        const c_middleRight = ceiling.points[6];
        const c_quarterBackRight = ceiling.points[7];
       // middleRight,
    ...
    #getPolys(world) {
        ...
        for (const poly of buildingPolys) {
            poly.fill = "rgba(150, 150, 150, 0.8)";
            poly.stroke = "rgba(150, 150, 150, 0.8)";
        }
